...
class DecisionTree:
    ...
    def predict_probability(self, depth, data):
        if self.is_leaf or depth == 0:
            return np.zeros(data.shape[0]) + self.probability
        new_tag = np.zeros(data.shape[0])
        l_data = data[:, self.tag_id] <= self.threshold
        new_tag[l_data] = self.left.predict_probability(depth - 1, data[l_data])
        r_data = data[:, self.tag_id] > self.threshold
        new_tag[r_data] = self.right.predict_probability(depth - 1, data[r_data])
        return new_tag
